...
describe('Routes for User Management Screen', () => {
    ...
    test('responds to /readUser', async () => {
        const res = await request(app).get('/readUser').set("Authorization", "Bearer " + adviserAccessToken);
        expect(res.body.status).toEqual(STATUS_CODE.SUCCESS);
    ...
    test('responds to /visionsNums', async () => {
        const res = await request(app).get('/visionsNums').set("Authorization", "Bearer " + adviserAccessToken);
        expect(res.body.status).toEqual(STATUS_CODE.SUCCESS);
