...
/**
 * 
 * @returns all first-year students who have a absent record in the past week 
 */
const getMissingFy = async () =>{
    const query = 
    `
        SELECT * FROM fy_attendance WHERE 
        date >= DATE_SUB(CURDATE(), INTERVAL 7 DAY) AND date <= CURDATE() 
        AND attendance ="absent"
    )`;

    const checkFy = new Promise((resolve, reject) => {
        connection.query(query, (err, res) => {
          if (err) reject(err);
          else resolve(res);
        })
    ...
    try {
        const missingFy = await checkFy;
        return ({ status: STATUS_CODE.SUCCESS, missingFy});
    } catch (e) {
        console.log(e);
        return ({ status: STATUS_CODE.ERROR});
    }
 ...
 */
exports.sendAdviserWeeklyReport = async () =>{
    try {
        const vuceptors = await getVUCeptorThatUnlogFY();
        const students = await getMissingFy();
        const email = await getAdvisorEmail();
        if (data.status === STATUS_CODE.SUCCESS && email.status === STATUS_CODE.SUCCESS){
            const html = `<p> Below are the VUceptors who have yet logged their visions attendance in the past week: </p>` 
            + tableify (vuceptors.vuceptorsWhoUnlogged)
            + `<p> Below are students who have absent records in the past week: </p>` 
            + tableify (students.missingFy);
            const mailOptions = {
