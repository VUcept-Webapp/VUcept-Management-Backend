...
const {STATUS_CODE,SORT_ORDER, ATTENDANCE} = require('./constants');
...
exports.readAttendance = async (param, table) =>{
    const tempWhere = table == 'vu_attendance' ? formPresenceClauseforVU(param, table) : formAbsenceClauseforFY(param, table);
    ...
    `SELECT name, email, visions, title AS event, mandatory, attendance AS status FROM ` + table
    + where + orderBy + ' LIMIT ' + row_num + ' OFFSET ' + row_start :
    `SELECT name, email, visions, title AS event, attendance AS status FROM ` + table
    + where + orderBy + ' LIMIT ' + row_num + ' OFFSET ' + row_start;
